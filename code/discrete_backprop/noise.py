"""  Noise functions for perturb-and-MAP.  ith thanks to Pasquale Minervini
from whose I-MLE repo this is adapted
"""
...
from torch.distributions import AffineTransform, SigmoidTransform, \
...
class NoNoise(BaseNoiseDistribution):
    def __init__(self,
                 location: float = 0.0,
                 scale: float = 1.0,
                 device: Optional[torch.device] = None):
...
class LogisticNoiseDistribution(BaseNoiseDistribution):
    def __init__(self,
                 location: float = 0.0,
                 scale: float = 1.0,
                 device: Optional[torch.device] = None):
        ...
        self.distribution = TransformedDistribution(
