...
# from src.dirichlet import dirichlet_weights_noise
...
class SyntheticDataset:
    ...
    def __init__(
            self, n, d, graph_type, degree, noise_type, B_scale, seed=1,
            perm=True):
        """Initialize self.
        ...
        # self.B_ranges = ((B_scale * -2.0, B_scale * 2.0), ) # Ch: elim bounding away
    ...
    def _setup(self):
        ...
        self.B_bin = SyntheticDataset.simulate_random_dag(
            self.d, self.degree, self.graph_type, self.rs, self.perm)
        if self.noise_type == "gaussian_dirichlet":
            raise NotImplementedError
            # self.B, self.noise_sds = dirichlet_weights_noise(
            #     self.B_bin, self.rs)  # Note that in the Dirichelt
            ...
            """  if noise_type == 'gaussian_ev':
                    # Gaussian noise with equal variances
                    N_i = rs.normal(scale=B_scale, size=n)
            """
            # Check that weights plus noise add to 1:
            # B_cols = self.B
            # noise = self.noise_sds
            # check = np.square(B_cols).sum(axis=0) + noise ** 2
        else:
            self.B, self.noise_sds = SyntheticDataset.simulate_weight(
                self.B_bin, self.B_ranges, self.rs), None
        # noinspection PyTypeChecker
        self.X = SyntheticDataset.simulate_linear_sem(
            self.B, self.n, self.noise_type, self.rs,
            self.B_scale, self.noise_sds)
    ...
    def simulate_random_dag(
            d, degree, graph_type, rs=np.random.RandomState(1), perm=True):
        """Simulate random DAG.
        ...
        Returns:
            numpy.ndarray: [d, d] binary adjacency matrix of DAG.
        """
        # B_bin = np.diag(np.ones(d), k=1)  # For chain graph
        # return B_bin                      # For chain graph
    ...
    def simulate_linear_sem(
            B, n, noise_type, rs=np.random.RandomState(1),
            B_scale=1., noise_sds=None):
        ...
        """
        def _simulate_single_equation(X, B_i, noise_sd):
            ...
            elif noise_type == 'gaussian_dirichlet':
                N_i = rs.normal(scale=noise_sd, size=n)
            else:
                raise ValueError("Unknown noise type.")
            return X @ B_i + N_i

        d = B.shape[0]
        if noise_type == 'gaussian_dirichlet':
            assert noise_sds is not None
        else:
            noise_sds = np.zeros(d)
        X = np.zeros([n, d])
        G = nx.DiGraph(B)
        ...
        for i in ordered_vertices:
            ...
            X[:, i] = _simulate_single_equation(
                X[:, parents], B[parents, i], noise_sds[i])
...
if __name__ == '__main__':
    n, d = 1000, 10_000
    graph_type, degree = 'SF', 4    # ER2 graph
    B_scale = 1.0
